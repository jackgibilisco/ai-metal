...
A macOS-only Metal renderer (plus an OpenGL 4.1 build at pixel parity) in
C-style C++ / Objective-C++, being grown into
...
`newLibraryWithSource:` (the GLSL likewise lives in `src/renderer_gl.cpp`). `clang++` alone (via the Xcode command line tools)
is enough to build.
...
headless unit tests (build them standalone) and the backend parity harness:
`make parity-test` must pass after any renderer or UI change (Metal is the
reference; GL must match within 1/255). In fullscreen an in-app menu strip appears at the top
...
backend in `src/gpu_gl.h`. A Windows port
...
| `src/renderer_common.h`/`.cpp`, `src/font_atlas.*` | pure C++ (+ CoreText) | what both backends share: mesh data, orbit camera, AO samples, overlay geometry, SDF glyph atlas bake |
...
| `src/renderer_gl.cpp`, `src/ui_render_gl.cpp`, `src/gpu_gl.h`, `src/gl_shader.*` | OpenGL 4.1 core, no AppKit | GL backend, same passes as Metal; no GPU timings (Apple GL timer queries read 0) |
